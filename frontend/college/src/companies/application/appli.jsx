...
export const OwnerApplicationList = () => {
    ...
    useEffect(() => {
        const fetchApplications = async () => {
            try {
                ...
                const appliedApplications = response.data.filter(app => app.status === 'Applied');
    ...
    const handleRe = async (applicationId,userId) => {
        try {
            await axios.patch(`http://localhost:8081/api/ownerapplications/${applicationId}`, { status: 'Interview' });
            const response = await axios.get('http://localhost:8081/api/ownerapplications', {
                params: { name },
            });
    ...
    const sendConfirmationEmail = async (userId) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8081/send-confirmation-applied', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userEmail: ownerEmail })
            });
                                                ...
                                                <tr key={application._id} className="border-b hover:bg-gray-100">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <button
                                                            ...
                                                            onClick={() => handleRe(application._id,application.userId)}>ACCEPT</button>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
